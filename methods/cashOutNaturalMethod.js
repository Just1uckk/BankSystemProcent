...
const cashOutNaturalMethod = (element, operationInfo,limit) => {
    ...
    if (!limit.length) {
        if (element.operation.amount <= operationInfo.week_limit.amount) {
            ...
            commission = (element.operation.amount - operationInfo.week_limit.amount) * (operationInfo.percents/100)
            limit.push({
                user_id : element.user_id,
                totalCash : element.operation.amount,
                week
        ...
        const index = limit.findIndex((el)=>{if(el.user_id === element.user_id){
            return true
        }})
        if (index === -1) {
            if (element.operation.amount <= operationInfo.week_limit.amount) {
            ...
            if (pastUser.week === week) {
                // если платеж проведен на одной неделе
                if (pastUser.totalCash>operationInfo.week_limit.amount ) {
                    commission = element.operation.amount * (operationInfo.percents/100)
                    limit.push({
                        user_id : element.user_id,
                        totalCash : pastUser.totalCash+element.operation.amount,
                        week
                    })
                } else if ((pastUser.totalCash + element.operation.amount)>operationInfo.week_limit.amount) {
                    commission = (pastUser.totalCash + element.operation.amount - operationInfo.week_limit.amount) * (operationInfo.percents/100)
                    limit.push({
                        user_id : element.user_id,
                        totalCash : pastUser.totalCash+element.operation.amount,
                        week
